...
const AdminPage: React.FC = () => {
  ...
  const loadCurrentPhase = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/admin/phase');
  ...
  const loadKhawatir = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/admin/khawatir');
  ...
  const changePhase = async (newPhase: number) => {
    try {
      await axios.post('http://localhost:5000/api/admin/phase', {
        phase: newPhase
      });
  ...
  const deleteKhatira = async (id: number) => {
    setLoading(true);
    try {
      await axios.delete(`http://localhost:5000/api/admin/khawatir/${id}`);
  ...
  const clearAllKhawatir = async () => {
    setLoading(true);
    try {
      await axios.delete('http://localhost:5000/api/admin/khawatir/clear-all');
      setKhawatir([]);
